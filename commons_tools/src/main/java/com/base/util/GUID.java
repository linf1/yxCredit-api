...
/**
  ...
  As such,  GUIDs are 16 byte arrays with a convenient
  GUID class wrapper.

  前4个字节是IP地址，接着8个字节是系统时间，最后4个字节是计数器。
  在数据库中存储可以使用 char(38) 类型，保存如下字符串
  {bfa78274-0000-0000-6661-bea400000003}，可以由 toString()方法得到。
  也可以用getData() 获得 byte[16] 进行处理。
*/
