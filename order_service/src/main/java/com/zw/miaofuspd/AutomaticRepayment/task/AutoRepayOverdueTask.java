...
/**
 * @Author xiahaiyang
 * @Create 2018年1月29日14:12:21
 **/
public class AutoRepayOverdueTask extends AbsTask{
    ...
    public void doWork() throws Exception {
        logger.info("定时执行成功");
        //获取所有逾期的还款计划
        String repaySql ="SELECT o.ID AS orderId, r.id AS repaymentId, r.repayment_amount AS" +
                " repaymentAmount,r.pay_count, r.state AS repayState, o.state AS orderState, r.pay_time AS payTime," +
                " o.USER_ID AS userId FROM mag_order o JOIN mag_repayment r ON r.order_id = o.ID AND" +
                " r.state = '3' WHERE o.state = '5' AND o.commodity_state IN ('19', '20')AND o.order_type = '2'";
        ...
        if (repayList != null && repayList.size() > 0) {
            for (int i = 0; i < repayList.size(); i++) {
                ...
                Double penalty = (null == map.get("penalty") || "".equals(map.get("penalty"))) ? 0d : Double.valueOf(map.get("penalty").toString());//逾期利息
                Double defaultInterest = (null == map.get("defaultInterest") || "".equals(map.get("defaultInterest"))) ? 0d : Double.valueOf(map.get("defaultInterest").toString());//罚息
                Double amount = repaymentAmount+penalty+defaultInterest;//应还总金额 = 应还金额+服务包金额
              ...
              //  String txn_amt = String.valueOf(new BigDecimal(amount).multiply(BigDecimal.valueOf(100)).setScale(0,BigDecimal.ROUND_HALF_UP));//支付金额转换成分
                ...
                try {
                    ...
                    if ("0000".equals(respCode) || "BF00114".equals(respCode)) {
                        ...
                        contentMsg = contentMsg.replace("$contractNo$", contractNo).replace("$payCount$", nowPeriods);
...
//                //更新还款计划表 将状态更新为还款确认中，然后在定时任务里面查询处理
                        String repaymentSql = "update mag_repayment set state ='4', actual_amount='"+amount+"',actual_time= '" + actualTime + "' where id ='" + repaymentId + "'";
                        sunbmpDaoSupport.exeSql(repaymentSql);
                        state = "0";//待确认
                    } else {//这种交易失败被认定为失败
                        String repaymentSql = "update mag_repayment set state ='1' where id ='" + repaymentId + "'";
                        sunbmpDaoSupport.exeSql(repaymentSql);
                        AppUserInfo userInfo = iUserService.getUserByOrderId(orderId);
                        ...
                        //发送短信
                        String contentMsg = iDictService.getDictInfo("短信模板","xshksb");
                        contentMsg = contentMsg.replace("$contractNo$", contractNo).replace("$payCount$", nowPeriods);
                        sendMessage(orderId, contentMsg);
                        state = "2";//失败
                    }
