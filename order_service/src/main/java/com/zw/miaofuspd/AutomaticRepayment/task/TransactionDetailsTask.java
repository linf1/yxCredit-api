 ...
 **/
public class TransactionDetailsTask extends AbsTask {
    ...
    public void doWork() throws Exception {
        logger.info("------------------还款确认中跑批----------------");
        String detailsSql = "select id,batch_no,amount,derate_id,derate_amount,repayment_id,order_id,baofu_mag,baofu_code,type,state,creat_time from  mag_transaction_details  where state ='0' and type !='0' and source='1'";
        List list = this.sunbmpDaoSupport.findForList(detailsSql);
        if(list!=null && list.size()>0){
            for (int i = 0; i < list.size();i++){
                Map detailMap =(Map)list.get(i);
                String origTransId = detailMap.get("batch_no").toString();
                String repaymentId = null == detailMap.get("repayment_id") ? "" : detailMap.get("repayment_id").toString();
                ...
                if("S".equals(jsonObject.getString("order_stat"))){
                    ...
                    {//计划还款
                        ...
                        //判断是否是提前结清
//                        String repaySql = "select settle_id as settleId,settle_type as settleType from mag_repayment where id = '"+repaymentId+"'";
//                        Map repayMap = sunbmpDaoSupport.findForMap(repaySql);
//                        String settleType = repayMap.get("settleType").toString();
//                        if (!"0".equals(settleType)){
//                            String settleId = repayMap.get("settleId").toString();
//                            String settleSql = "update mag_settle_record set state = '2' where id = '"+settleId+"'";
...
//                        }
                ...
                String detailSql = "update mag_transaction_details set state = '"+state+"' where batch_no = '"+origTransId+"'";
                sunbmpDaoSupport.exeSql(detailSql);
            }
        }
    }
