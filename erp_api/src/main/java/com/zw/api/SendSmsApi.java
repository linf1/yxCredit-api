...
//        String msg = "【秒付金服】您的验证码为$code$,用于进行网站用户注册功能,为了账户安全,请勿泄露,本次验证码有效时间为120秒。全国服务热线:400-0686-600";
        try {
            TraceLoggerUtil.info("发送短信Url--->"+host);
            SmsSendRequest smsSingleRequest = new SmsSendRequest(account, password, msg, phone,report);
            TraceLoggerUtil.info("请求之前数据-->"+smsSingleRequest);
            String requestJson = JSON.toJSONString(smsSingleRequest);
            String response = ChuangLanSmsUtil.sendSmsByPost(host, requestJson);
            TraceLoggerUtil.info("接收到的结果--->"+response);
            SmsSendResponse smsSingleResponse = JSON.parseObject(response, SmsSendResponse.class);
            TraceLoggerUtil.info("接收到的结果--->"+smsSingleResponse);
            ...
            TraceLoggerUtil.error("发送短信接口--->", ex);
            throw ex;
        }
    }
    public static Map sendAliyun(String host, Map map) {
        Map returnMap = new HashMap();
        ...
        try {
            ...
            TraceLoggerUtil.info("接收到的结果--->"+jsonObject1);
    ...
    public static void main(String[] args) throws Exception{
        String msg = "【可以贷】您的验证码为$code$,用于进行网站用户注册功能,为了账户安全,请勿泄露,本次验证码有效时间为120秒。全国服务热线:400-0686-600";
        String code = "123455";
